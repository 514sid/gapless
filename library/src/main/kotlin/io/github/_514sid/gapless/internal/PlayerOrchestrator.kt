...
internal class PlayerOrchestrator(
    ...
    var onPreloadMissed: ((String) -> Unit)? = null
    ...
    private fun play(item: PlaybackItem) {
        when (item) {
            ...
            is PlaybackItem.Image -> {
                if (!image.isPrepareComplete) onPreloadMissed?.invoke(item.assetId)
                image.play(item)
                video.clear()
                web.clear()
            ...
            is PlaybackItem.Web -> {
                if (!web.pageCommitted) onPreloadMissed?.invoke(item.assetId)
                web.play(item)
                video.clear()
                image.clear()
